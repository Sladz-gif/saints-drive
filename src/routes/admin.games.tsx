...
const GAMES = [
  ...
  {
    id: "g2",
    name: "Match the Brand",
    type: "Memory",
    plays: 8934,
    status: "Active",
    featured: true,
  },
  { id: "g3", name: "Price Guess", type: "Trivia", plays: 5678, status: "Active", featured: false },
  { id: "g4", name: "Speed Racer", type: "Racing", plays: 2345, status: "Draft", featured: false },
  { id: "g5", name: "Word Search", type: "Puzzle", plays: 1234, status: "Active", featured: false },
];
    ...
    <AdminLayout title="Games">
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="font-display text-2xl tracking-wider">GAMES</div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Games", value: "12", icon: Gamepad2, color: "text-white" },
            { label: "Featured", value: "4", icon: TrendingUp, color: "text-primary" },
            { label: "Total Plays", value: "45.2K", icon: Play, color: "text-success" },
            { label: "Active", value: "8", icon: Eye, color: "text-info" },
          ].map((s) => (
            <div
              key={s.label}
        ...
        <div className="bg-admin-surface border border-admin-border">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[10px] font-mono uppercase tracking-widest text-muted-foreground border-b border-admin-border">
                <th className="px-4 py-3 w-10">
                  <input
                    type="checkbox"
                    checked={selectedIds.size === filteredGames.length && filteredGames.length > 0}
            ...
            <tbody>
              {filteredGames.map((g) => (
                <tr key={g.id} className="border-b border-admin-border/60 hover:bg-admin-bg/60">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-display tracking-wide">{g.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs">{g.type}</td>
                  <td className="px-4 py-3 font-mono">{g.plays.toLocaleString()}</td>
