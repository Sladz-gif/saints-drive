      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-4">
        <div className="lg:col-span-2 bg-admin-surface border border-admin-border p-5">
            ...
            {Array.from({ length: 30 }).map((_, i) => (
              <div key={i} className="flex-1 flex flex-col gap-0.5">
                ...
                <div
                  ...
                  style={{ height: `${10 + Math.abs(Math.cos(i)) * 40}%` }}
                />
              </div>
            ))}
          </div>
          <div className="mt-3 flex gap-4 text-xs font-mono">
            <span className="inline-flex items-center gap-1.5">
              <span className="w-2 h-2 bg-primary" /> Listings
            </span>
            <span className="inline-flex items-center gap-1.5">
              <span className="w-2 h-2 bg-foreground/30" /> Users
            </span>
          </div>
        </div>
        ...
        <div className="bg-admin-surface border border-admin-border p-5">
          ...
          <div
            ...
            style={{
              background:
                "conic-gradient(var(--color-primary) 0 55%, var(--color-success) 55% 78%, var(--color-info) 78% 92%, var(--color-muted) 92% 100%)",
            }}
          >
          ...
          <div className="mt-4 space-y-2 text-sm">
            {[
              ...
              ["Rentals", "23%", "success"],
              ["Auctions", "14%", "info"],
              ["Ads", "8%", "muted-foreground"],
            ].map(([k, v, c]) => (
              <div key={k} className="flex justify-between">
                <span className={`text-${c}`}>● {k}</span>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mt-4">
        <div className="bg-admin-surface border border-admin-border p-5">
          ...
          <div className="space-y-3 text-sm">
            {[
              ...
              ["32m", "User registered", "Kojo Mensah"],
              ["1h", "Listing sold: BMW M340i", "Accra"],
            ].map(([t, msg, meta], i) => (
              <div
                key={i}
                className="flex items-start gap-3 py-2 border-b border-admin-border/60 last:border-0"
              >
        ...
        <div className="bg-admin-surface border border-admin-border p-5">
          ...
          <div className="space-y-2">
            {[
              ["23 listings", "awaiting review"],
              ["2 refund requests", "to approve"],
              ["1 expired ad campaign", "needs renewal"],
            ].map(([k, v], i) => (
              <div
                key={i}
